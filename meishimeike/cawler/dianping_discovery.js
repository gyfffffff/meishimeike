...
function parseSeed(body) {
    ...
    all_class.each(function (index, elememt) {   // 遍历每个店家
        ...
        try {
            ...
            if (class_name == "推荐" || class_name == "美食"){  // ||class_name == "丽人"
...
function parseItem(body, url){
    ...
    var laiyuan = "点评discovery"

    var fetchAddSql = "INSERT INTO fetches(_url,url_shop,shop_name,score,leixing,renjun,dizhi,pics,_text,_time,fensi,author_dizhi,crawltime,laiyuan) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)";  // 13个字段,13?
    var fetchAddSql_Params = new Array()
    try{
        url_shop = "https:"+$(".related-shop-item a").attr("href")
        shop_name = $(".shop-name").text()
        try{
            score = body.match(/sml-str(\d+)/)[1]/10
            leixing = $(".area-info").text().split("|")[0].trim()
            dizhi = $(".area-info").text().split("|")[1].trim()
        }catch(e){}
        author_dizhi = $("#__dx_app__ > div.feed-detail-wrap > div > div.feed-detail-right > div.author-info-module > div.author-info > div.other-info-wrapper > span.city").text()
        renjun = $(".avg-info").text()
        var emojiRegex = /[\uD800-\uDBFF][\uDC00-\uDFFF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|\u200D/g;
        _text = ($(".content-module h1").text() + " " + $(".content-module p").text()).replace(emojiRegex, '')
        ...
        fetchAddSql_Params = [url, url_shop, shop_name, score, leixing, renjun, dizhi, pics.join("; "), _text, _time, fensi, author_dizhi,crawltime,laiyuan]
        // console.log(fetchAddSql_Params)
        mysql.query(fetchAddSql, fetchAddSql_Params, (qerr, vals, fields) => {
            if(qerr){console.log(qerr)}
        })
    } catch(e) {
        // console.log(url+" 解析出错："+ e)
    }
}
